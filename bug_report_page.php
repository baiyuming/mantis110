...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		<select <?php echo helper_get_tab_index() ?> name="severity" id="severity" onchange="setSeverity();">
...
<script type="text/javascript">
	...
	category["20"] = 10;//高
	message["20"] = "设计不善产生的缺陷";
	category["40"] = 10;
	message["40"] = "系统改版、升级后功能遗漏、错误";
	category["50"] = 10;
	message["50"] = "因需求、设计、开发不善产生的业务逻辑错误";
	category["80"] = 10;
	message["80"] = "服务器端达不到性能指标、客户端内存、cpu等占有率过高，或引起的其他不稳定及冲突";

	category["10"] = 10;//中
	message["10"] = "线上既存的缺陷，或者历史原因产生的缺陷";
	category["30"] = 20;
	message["30"] = "页面Javascript错误";
	category["60"] = 20;
	message["60"] = "部署文档有误、测试人员配置错误、信息不畅引起的环境配置问题，线上环境与测试环境差异造成的问题";

	category["70"] = 30;//低
	message["70"] = "页面、控件、焦点、格式不美观、不合理、不统一、不全面，提示信息不准确、不够全面，文字性的错误";

	category["120"] = 40;//轻微
	message["120"] = "测试过程中自己的想法或者更加合理的建议";
	category["90"] = 50;//无
	message["90"] = "部署文档有误、测试人员配置错误、信息不畅引起的环境配置问题，线上环境与测试环境差异造成的问题";
	category["100"] = 50;
	message["100"] = "开发过程中项目各角色提出的新需求";
	category["110"] = 50; 
	message["110"] = "数据库中因脏数据产生的错误";
	category["130"] = 50;
	message["130"] = "不可再现、测试理解错误、测试用例错误";

	category["140"] = 50;
	message["140"] = "story内部和ui错误";
	category["150"] = 50;
	message["150"] = "组件或者服务之间接口问题";
	category["160"] = 50;
	message["160"] = "分支未考虑逻辑";
	document.getElementById("severity").value = 30;
	
	function setProperty(){
			var reval = document.getElementById("reproducibility").value;
			var selectVal = category[reval];
			var o = document.getElementById("severity");
			for( i=0;i<o.options.length;i++){
					if(o.options[i].value == selectVal){
						o.options[i].selected = true;
						document.getElementById("notice").innerHTML = message[reval];
					}
			}			
	}
	...
	function setSeverity(){
		...
		var reval = document.getElementById("reproducibility").value;
		var o = document.getElementById("severity");
		if(category[reval] != severity){
			for( i=0;i<o.options.length;i++){
					if(category[reval] == o.options[i].value){
						o.options[i].selected = true;	
					}
			}			
		}
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		<textarea <?php echo helper_get_tab_index() ?> name="description" cols="80" rows="8"><?php echo $f_description ?></textarea>
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		<textarea <?php echo helper_get_tab_index() ?> name="additional_info" cols="80" rows="1"><?php echo $f_additional_info ?></textarea>
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
 ...
 <!-- <tr><?php echo helper_alternate_class() ?>
	 <td class="category">
		<span class="required">*</span>RTX notify:
	</td>
	<td>
		<input type="text" name="employeeids" /> (employeeid no. by "," blank)
	</td>
</tr>-->
