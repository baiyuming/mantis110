<?php
	...
	function filter_get_bug_rows( &$p_page_number, &$p_per_page, &$p_page_count, &$p_bug_count, $p_custom_filter = null, $p_project_id = null, $p_user_id = null, $p_show_sticky = null ) {
		...
		if ( 'simple' == $t_filter['_view_type'] ) {
			...
			//echo "this_status:".$t_this_status;
			//echo "hide_status:".$t_this_hide_status.'<br/>';
			...
			if ( $t_any_found ) {
				foreach( $t_available_statuses as $t_this_available_status ) {
					if ( $t_this_hide_status >= $t_this_available_status) {//> modify >=
						$t_desired_statuses[] = $t_this_available_status;
					}
					if($t_this_hide_status == 10){//add
						$t_desired_statuses[] = 10;//add
					}//add
				}
			} else {
				//echo "ffff:".$t_this_status;
		...
		//echo "111111111====".count( $t_desired_statuses );
		if ( count( $t_desired_statuses ) > 0 ) {
			...
			//var_dump($t_clauses);
			//echo "bbbb=". count($t_clauses);
			if ( 1 < count( $t_clauses ) ) {
				//echo "yyyyyy".$t_this_hide_status;
				//array_push( $t_where_clauses, "( $t_bug_table.status in (". implode( ', ', $t_clauses ) .") )" );
				array_push( $t_where_clauses, "( $t_bug_table.status <> ".  $t_this_hide_status  ." )" );
			} else {
				//echo "oooooooooooo";
		...
		if ( !$t_any_found ) {
			...
			}elseif ($t_clauses[0]==100){
				array_push( $t_where_clauses, "( $t_bug_table.resolution in (10,20,30,50,100) )" );
			}elseif($t_clauses[0]==110){
				array_push( $t_where_clauses, "( $t_bug_table.resolution in (40,60,70,110) )" );
			}else {
...
<?php
	...
	function print_filter_reporter_id(){
		...
		?>
		<select <?php PRINT $t_select_modifier;?> name="reporter_id[]" id="reporter_id" onchange="setValue()">
		<?php
		...
		<input type="text" name="uname" onblur="changetText()" id="uname"/>
