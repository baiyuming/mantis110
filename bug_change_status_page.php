...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		<select name="handler_id"  id="handler_id" onchange="setValue();">
			<option value="0"></option>
			<?php print_assign_to_option_list( $t_bug->handler_id, $t_bug->project_id ) ?>
		</select>
		<input type="text" id="handlerid" onblur="changetText();"/>
	</td>
</tr>
	...
	function setValue(){
		   ...
		   for(var i=0;i<teamArr.options.length;i++){
		 	   if(teamArr[i].selected){
		 		  document.getElementById("handlerid").value = teamArr[i].text;
				 }
		   }  
	 }
	 
		function changetText(){
			var uname = document.getElementById("handlerid").value; 
